...
var cache = flatCache.load(‘cacheId’);</p>

<p>// sets a key on the cache
cache.setKey(‘key’, { foo: ‘var’ });</p>

<p>// get a key from the cache
cache.getKey(‘key’) // { foo: ‘var’ }</p>

<p>// fetch the entire persisted object
cache.all() // { ‘key’: { foo: ‘var’ } }</p>

<p>// remove a key
cache.removeKey(‘key’); // removes a key from the cache</p>

<p>// save it to disk
cache.save(); // very important, if you don’t save no changes will be persisted.
// cache.save( true /* noPrune */) // can be used to prevent the removal of non visited keys</p>

<p>// loads the cache from a given directory, if one does
// not exists for the given Id a new one will be prepared to be created
var cache = flatCache.load(‘cacheId’, path.resolve(‘./path/to/folder’));</p>

<p>// The following methods are useful to clear the cache
// delete a given cache
flatCache.clearCacheById(‘cacheId’) // removes the cacheId document if one exists.</p>
