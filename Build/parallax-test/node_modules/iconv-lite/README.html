<h2>Pure JS character encoding conversion <a href="https://travis-ci.org/ashtuchkin/iconv-lite"><img src="https://travis-ci.org/ashtuchkin/iconv-lite.svg?branch=master" alt="Build Status" /></a></h2>
...
<p>### Basic API
```javascript
var iconv = require(‘iconv-lite’);</p>

<p>// Convert from an encoded buffer to js string.
str = iconv.decode(new Buffer([0x68, 0x65, 0x6c, 0x6c, 0x6f]), ‘win1251’);</p>

<p>// Convert from js string to an encoded buffer.
buf = iconv.encode(“Sample input string”, ‘win1251’);</p>

<p>// Check if encoding is supported
...
<p>// Decode stream (from binary stream to js strings)
http.createServer(function(req, res) {
    var converterStream = iconv.decodeStream(‘win1251’);
    req.pipe(converterStream);</p>

<pre><code>converterStream.on('data', function(str) {
    console.log(str); // Do something with decoded strings, chunk-by-chunk.
}); });
</code></pre>

<p>// Convert encoding streaming example
fs.createReadStream(‘file-in-win1251.txt’)
    .pipe(iconv.decodeStream(‘win1251’))
    .pipe(iconv.encodeStream(‘ucs2’))
    .pipe(fs.createWriteStream(‘file-in-ucs2.txt’));</p>

<p>// Sugar: all encode/decode streams have .collect(cb) method to accumulate data.
http.createServer(function(req, res) {
...
Buffer.byteLength(str, ‘us-ascii’);</p>
...
<p>fs.createReadStream(“file.txt”, “shift_jis”);</p>

<p>// External modules are also supported (if they use Node primitives, which they probably do).
...
<p>$ # To view performance:
$ node test/performance.js</p>

<p>$ # To view test coverage:
$ npm run coverage
