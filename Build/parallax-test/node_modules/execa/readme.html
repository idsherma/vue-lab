...
<p>```js
const execa = require(‘execa’);</p>

<p>execa(‘echo’, [‘unicorns’]).then(result => {
	console.log(result.stdout);
	//=> ‘unicorns’
});</p>

<p>// pipe the child process stdout to the current stdout
execa(‘echo’, [‘unicorns’]).stdout.pipe(process.stdout);</p>

<p>execa.shell(‘echo unicorns’).then(result => {
	console.log(result.stdout);
	//=> ‘unicorns’
});</p>

<p>// example of catching an error
...
const getStream = require(‘get-stream’);</p>

<p>const stream = execa(‘echo’, [‘foo’]).stdout;</p>

<p>stream.pipe(process.stdout);</p>

<p>getStream(stream).then(value => {
