...
<p>````javascript
#!/usr/bin/env node
var argv = require(‘yargs’).argv;</p>

<p>if (argv.ships > 3 && argv.distance < 53.5) {
...
<p>VERBOSE_LEVEL = argv.verbose;</p>

<p>function WARN()  { VERBOSE_LEVEL >= 0 && console.log.apply(console, arguments); }
function INFO()  { VERBOSE_LEVEL >= 1 && console.log.apply(console, arguments); }
function DEBUG() { VERBOSE_LEVEL >= 2 && console.log.apply(console, arguments); }</p>

<p>WARN(“Showing only important stuff”);
...
<p>console.log(“The area is:”, argv.w * argv.h);
````</p>
...
<p>var fs = require(‘fs’);
var s = fs.createReadStream(argv.file);</p>
...
Commands:
  ...
  configure <key> [value]  Set a config variable  [aliases: config, cfg]</key></p>

<p>Options:
  –help  Show help                                            [boolean]</p>

<p>$ ./svc.js cfg concurrency 4
setting concurrency to 4</p>
...
<p>```js
// my-module.js
exports.command = ‘get <source /> [proxy]’</p>

<p>exports.describe = ‘make a get HTTP request’</p>
...
<p>exports.handler = function (argv) {
...
<p>Missing required arguments: run, path
...
<p>.demandCommand(min, [max], [minMsg], [maxMsg])
——————————</p>

<p>Demand in context of commands. You can demand a minimum and a maximum number a user can have within your program, as well as provide corresponding error messages if either of the demands is not met.
<code>javascript
require('yargs')
  .command({
    command: 'configure <key> [value]',
...
<p>You need at least one command before moving on
```</p>
...
***
    Heroes:
      –batman  world’s greatest detective</p>
...
<p>```shell
./test.js - follow ye instructions true</p>

<p>Choose yer command:
  run  Arrr, ya best be knowin’ what yer doin’</p>

<p>Options for me hearties!
  –option, -o  ‘tis a mighty fine option               [requi-yar-ed]
  –help        Parlay this here code of conduct             [boolean]</p>

<p>Ex. marks the spot:
  test.js run foo  shiver me timbers, here’s an example for ye</p>

<p>Ye be havin’ to set the followin’ argument land lubber: option
```</p>
...
<p>```js
// providing the <code>fn</code> argument to <code>parse()</code> runs yargs in headless mode, this
...
const parser = yargs
  ...
  .help()</time></restaurant></p>

<p>parser.parse(bot.userText, function (err, argv, output) {
...
<p>```
$ node line_count.js
Missing argument value: f</p>

<p>Specify –help for available options
```</p>
