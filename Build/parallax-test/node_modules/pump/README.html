...
<p>var source = fs.createReadStream(‘/dev/random’)
var dest = fs.createWriteStream(‘/dev/null’)</p>

<p>pump(source, dest, function(err) {
...
<p>``` js
var transform = someTransformStream()</p>

<p>pump(source, transform, anotherTransform, dest, function(err) {
  console.log(‘pipe finished’, err)
})
```</p>
