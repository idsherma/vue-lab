...
<p>```js
var readPkg = require(‘read-pkg’);</p>

<p>readPkg().then(function (pkg) {
	console.log(pkg);
	//=> {name: ‘read-pkg’, …}
});</p>

<p>readPkg(__dirname).then(function (pkg) {
	console.log(pkg);
	//=> {name: ‘read-pkg’, …}
});</p>

<p>readPkg(path.join(‘unicorn’, ‘package.json’)).then(function (pkg) {
	console.log(pkg);
	//=> {name: ‘read-pkg’, …}
});
```</p>
