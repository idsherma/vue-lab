...
var send = require(‘send’)</p>

<p>var server = http.createServer(function onRequest (req, res) {
...
<p>// Default unknown types to text/plain
send.mime.default_type = ‘text/plain’</p>
...
<p>// Transfer arbitrary files from within /www/example.com/public/*
...
<p>// Custom directory handler
function directory (res, path) {
  var stream = this</p>

<p>// redirect to trailing slash for consistent url
  if (!stream.hasTrailingSlash()) {
    return stream.redirect(path)
  }</p>

<p>// get directory list
  fs.readdir(path, function onReaddir (err, list) {
    if (err) return stream.error(err)</p>

<pre><code>// render an index for the directory
