...
<p>```js
var Buffer = require(‘safe-buffer’).Buffer</p>

<p>// Existing buffer code will continue to work without issues:</p>

<p>new Buffer(‘hey’, ‘utf8’)
new Buffer([1, 2, 3], ‘utf8’)
new Buffer(obj)
new Buffer(16) // create an uninitialized buffer (potentially unsafe)</p>

<p>// But you can use these new explicit APIs to make clear what you want:</p>

<p>Buffer.from(‘hey’, ‘utf8’) // convert from many types to a Buffer
...
<p>const buf = Buffer.from(arr.buffer); // shares the memory with arr;</p>

<p>console.log(buf);
  // Prints: <Buffer 88 13 a0 0f></p>

<p>// changing the TypedArray changes the Buffer also
arr[1] = 6000;</p>
...
console.log(buf1.toString(‘ascii’));
  // prints: this is a tC)st</p>

<p>const buf2 = Buffer.from(‘7468697320697320612074c3a97374’, ‘hex’);
console.log(buf2.toString());
  // prints: this is a tést
```</p>
...
<p>```js
var buf = new Buffer(16)</p>

<p>// Immediately overwrite the uninitialized buffer with data from another buffer
...
<p>```js
var buf = Buffer.allocUnsafe(16) // careful, uninitialized memory!</p>
