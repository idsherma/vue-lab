...
<p>var program = require(‘commander’);</p>
...
<p>program
  ...
  .parse(process.argv);</p>

<p>console.log(‘you ordered a pizza with:’);
...
<p>console.log(‘you ordered a pizza’);
...
<p>function collect(val, memo) {
  memo.push(val);
  return memo;
}</p>

<p>function increaseVerbosity(v, total) {
  return total + 1;
}</p>
...
<p>program
  ...
  .parse(process.argv);</items></b></a></n></n></p>

<p>console.log(‘ int: %j’, program.integer);
...
program
  ...
  .parse(process.argv);</size></p>

<p>console.log(‘ size: %j’, program.size);
...
<p>if (typeof cmdValue === ‘undefined’) {
...
```
Angled brackets (e.g. <code><cmd></code>) indicate required input. Square brackets (e.g. <code>[env]</code>) indicate optional input.</p>

<h2>Git-style sub-commands</h2>

<p>```js
// file: ./examples/pm
var program = require(‘commander’);</p>
...
<p>```<br />
 $ ./examples/pizza –help</p>

<p>Usage: pizza [options]</p>

<p>An application for pizzas ordering</p>
...
<p>program
  ...
  .option(‘-B, –baz’, ‘enable some baz’);</p>

<p>// must be before .parse() since
// node’s emit() is immediate</p>

<p>program.on(‘–help’, function(){
...
<p>if (!process.argv.slice(2).length) {
  program.outputHelp(make_red);
}</p>

<p>function make_red(txt) {
...
<p>program
  ...
  .option('-T, --no-tests', 'ignore test hook');</path></path></p>
