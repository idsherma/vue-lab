...
<p>```js
const readPkg = require(‘read-pkg’);</p>
...
<p>readPkg(__dirname).then(pkg => {
	console.log(pkg);
	//=> {name: ‘read-pkg’, …}
});</p>

<p>readPkg(path.join(‘unicorn’, ‘package.json’)).then(pkg => {
	console.log(pkg);
	//=> {name: ‘read-pkg’, …}
});
```</p>
...
<p>Returns a <code>Promise</code> for the parsed JSON.</p>
