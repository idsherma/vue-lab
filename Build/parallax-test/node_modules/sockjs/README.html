...
<p>To install <code>sockjs-node</code> run:</p>

<pre><code>npm install sockjs
</code></pre>
...
var sockjs = require(‘sockjs’);</p>

<p>var echo = sockjs.createServer({ sockjs_url: ‘http://cdn.jsdelivr.net/sockjs/1.0.1/sockjs.min.js’ });
...
<p>The API design is based on the common Node API’s like
<a href="http://nodejs.org/docs/v0.5.8/api/streams.html">Streams API</a> or
<a href="http://nodejs.org/docs/v0.5.8/api/http.html#http.Server">Http.Server API</a>.</p>

<h3>Server class</h3>

<p>SockJS module is generating a <code>Server</code> class, similar to
...
<p>Once you have create <code>Server</code> instance you can hook it to the
...
<p>Where <code>options</code> can overshadow options given when creating <code>Server</code>
instance.</p>

<p><code>Server</code> instance is an
...
<p>All http requests that don’t go under the path selected by <code>prefix</code>
...
<code>installHandlers</code>.</p>

<h3>Connection instance</h3>

<p>A <code>Connection</code> instance supports
...
<a href="https://github.com/sockjs/sockjs-node/tree/master/examples/echo"><code>echo</code> example</a>
...
<code>/resource/<server_number>/<session_id>/transport</code>.  This is
useful for load balancers that support prefix-based affinity
(HAProxy does).</li>
  <li><code>JSESSIONID</code> cookie is being set by SockJS-node. Many load
...
<code>cookie:true</code> option to SockJS constructor.</li>
</ul>
...
<pre><code>make test_server
</code></pre>

<h3>SockJS-protocol Python tests</h3>

<p>To run it run something like:</p>

<pre><code>cd sockjs-protocol
make test_deps
...
<code>make serve</code>, which will automatically the server when you modify the
source code.</p>
