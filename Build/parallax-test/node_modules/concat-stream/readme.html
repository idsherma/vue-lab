...
<p>var readStream = fs.createReadStream(‘cat.png’)
var concatStream = concat(gotPicture)</p>

<p>readStream.on(‘error’, handleError)
readStream.pipe(concatStream)</p>

<p>function gotPicture(imageBuffer) {
  // imageBuffer is all of <code>cat.png</code> as a node.js Buffer
}</p>

<p>function handleError(err) {
...
<h2>var writable = concat(opts={}, cb)</h2>
