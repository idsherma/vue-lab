...
<p>var untar = pumpify(zlib.createGunzip(), tar.extract(‘output-folder’))
// you can also pass an array instead
// var untar = pumpify([zlib.createGunzip(), tar.extract(‘output-folder’)])</p>

<p>fs.createReadStream(‘some-gzipped-tarball.tgz’).pipe(untar)
```</p>
