...
<html lang="en">
<head>
    <title>Code coverage report for lib/spdy-transport/protocol/spdy/parser.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../../index.html">all files</a> / <a class='current-parent' href="index.html">lib/spdy-transport/protocol/spdy/</a> parser.js
    </h1>
    <div class='clearfix'>
...
484
485
486</td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
&nbsp;
Parser.prototype.setMaxHeaderListSize = <span class="fstat-no" title="function not covered" >function setMaxHeaderListSize (size) {</span>
  // http2-only
}
&nbsp;
// Only for testing
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!buffer.has(size)) { <span class="cstat-no" title="statement not covered" >return callback(new Error('KV OOB')) }</span>
&nbsp;
...
&nbsp;
    function readString () {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!buffer.has(size)) { <span class="cstat-no" title="statement not covered" >return null }</span>
      var len = self.version === 2
        ? buffer.readUInt16BE()
        : buffer.readUInt32BE()
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!buffer.has(len)) { <span class="cstat-no" title="statement not covered" >return null }</span>
&nbsp;
...
Parser.prototype.onRSTFrame = function onRSTFrame (body, callback) {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!body.has(8)) { <span class="cstat-no" title="statement not covered" >return callback(new Error('RST OOB')) }</span>
&nbsp;
  var frame = {
    type: 'RST',
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (flags &amp; 0x2) { <span class="cstat-no" title="statement not covered" >continue }</span>
&nbsp;
    var name = idMap[id]
&nbsp;
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!body.has(len)) { <span class="cstat-no" title="statement not covered" >return callback(new Error('X_FORWARDED host length OOB')) }</span>
&nbsp;
  callback(null, {
