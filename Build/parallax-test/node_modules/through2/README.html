...
fs.createReadStream(‘ex.txt’)
  .pipe(through2(function (chunk, enc, callback) {
    for (var i = 0; i < chunk.length; i++)
      if (chunk[i] == 97)
        chunk[i] = 122 // swap ‘a’ for ‘z’</p>

<pre><code>this.push(chunk)

callback()    }))   .pipe(fs.createWriteStream('out.txt'))   .on('finish', function () {
doSomethingSpecial()   }) ```
</code></pre>
...
<p>fs.createReadStream(‘data.csv’)
  .pipe(csv2())
...
<pre><code>callback()   }))   .on('data', function (data) {
all.push(data)   })   .on('end', function () {
