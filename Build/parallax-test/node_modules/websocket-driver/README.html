<h1>websocket-driver <a href="https://travis-ci.org/faye/websocket-driver-node"><img src="https://travis-ci.org/faye/websocket-driver-node.svg" alt="Build Status" /></a></h1>
...
var http = require(‘http’),
    websocket = require(‘websocket-driver’);</p>

<p>var server = http.createServer();</p>

<p>server.on(‘upgrade’, function(request, socket, body) {
  if (!websocket.isWebSocket(request)) return;</p>

<p>var driver = websocket.http(request);</p>

<p>driver.io.write(body);
  socket.pipe(driver.io).pipe(socket);</p>

<p>driver.messages.on(‘data’, function(message) {
...
<p>var server = net.createServer(function(connection) {
  var driver = websocket.server();</p>

<p>driver.on(‘connect’, function() {
...
<p>driver.on(‘close’, function() { connection.end() });
  connection.on(‘error’, function() {});</p>

<p>connection.pipe(driver.io).pipe(connection);</p>

<p>driver.messages.pipe(driver.messages);
});</p>

<p>server.listen(4180);
```</p>
...
<p>var driver = websocket.client(‘ws://www.example.com/socket’),
    tcp = net.connect(80, ‘www.example.com’);</p>

<p>tcp.pipe(driver.io).pipe(tcp);</p>

<p>tcp.on(‘connect’, function() {
  driver.start();
});</p>
...
<p>var driver = websocket.client(‘ws://www.example.com/socket’),
    ...
    tcp    = net.connect(80, ‘proxy.example.com’);</p>

<p>tcp.pipe(proxy).pipe(tcp, {end: false});</p>

<p>tcp.on(‘connect’, function() {
  proxy.start();
});</p>

<p>proxy.on(‘connect’, function() {
...
<ul>
  <li><b><code>driver.io</code></b> - this stream should be attached to an I/O socket like a
...
pipe this stream back into TCP to send outgoing frames.</li>
  <li><b><code>driver.messages</code></b> - this stream emits messages received over the
...
<h4><code>driver.on('open', function(event) {})</code></h4>

<p>Adds a callback to execute when the socket becomes open.</p>

<h4><code>driver.on('message', function(event) {})</code></h4>
...
<h4><code>driver.on('error', function(event) {})</code></h4>
...
<h4><code>driver.on('close', function(event) {})</code></h4>
...
<h4><code>driver.on('ping', function(event) {})</code></h4>
...
<h4><code>driver.on('pong', function(event) {})</code></h4>
...
<h4><code>driver.ping(string = '', function() {})</code></h4>
